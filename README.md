...
|--------|---------------|----------------------|
| **`sample/`** | One tiny synthetic workbook for the 60‑second test below. **Don't edit it.** | Ships with the repo |
| **`sample-workbooks/`** | Bigger, more realistic workbooks — **ships empty**, you supply them ([how](sample-workbooks/README.md)). | **You** — git‑ignored |
...
> **Rule of thumb:** kicking the tires → use `sample/`. Doing real work → put files in
> **`workbooks/`**. Output always lands in the folder you pass to `-Output` (e.g. `.\out`).
>
> **This repo ships no third-party Tableau files.** Everything under `sample/`, `sample-hard/`
> and `sample-tablecalc/` is generated by the engine's own synthetic fixtures. Workbooks from
> Tableau Public or a customer tenant belong to their authors, so you download those yourself —
> see [`sample-workbooks/README.md`](sample-workbooks/README.md).
...
**Try something more realistic.** The `sample/` file is deliberately tiny. To convert something
meatier, put real workbooks in **`sample-workbooks/`** — that folder ships empty because this repo
doesn't redistribute other people's dashboards. Two supported ways to fill it (download a viz you
like from Tableau Public, or generate a synthetic multi-source estate with one command) are in
[`sample-workbooks/README.md`](sample-workbooks/README.md). Then:
...
tool runs. For **real work** — workbooks you download from Tableau Cloud/Server, or a customer's
own exports — use the ready-made **`workbooks/`** folder: drop
...
BI/Fabric semantic model + an openable PBIP back. Where to put files (the bundled synthetic
sample, workbooks you supply in `sample-workbooks/`, or your own in `workbooks/`) is covered by
the folder map in [Quick start](#quick-start-get-a-working-result-in-60-seconds).
...
| `sample/` | `Superstore.tds` + `Superstore.twb` — a real-*shaped* but **synthetic** datasource + workbook, generated by the engine's own fixtures (offline; no live Tableau needed). The fixed **60-second Quick start** file. |
| `sample-workbooks/` | **Ships empty.** Where you put bigger, realistic workbooks you supply yourself — see [`sample-workbooks/README.md`](sample-workbooks/README.md). Git-ignored. |
...
These are unedited screenshots of the report produced by a single run against a 13-workbook
estate of real Tableau dashboards. Nothing here is hand-written.

**That estate is not in this repo, and the numbers below are therefore not reproducible
file-for-file.** The workbooks belong to their authors, so redistributing them here would be a
licensing problem — see [`sample-workbooks/README.md`](sample-workbooks/README.md) for how to
assemble a comparable corpus of your own. What *is* reproducible on a fresh clone is the
[Quick start](#quick-start-get-a-working-result-in-60-seconds) against the synthetic `sample/`
file, and the committed `output/` bundle it produces. Treat the estate figures as a measured
result we are reporting, not as a claim you can verify without your own workbooks.
